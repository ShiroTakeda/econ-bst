...
\begin{enumerate}
 ...
 \item Value of ``\texttt{absorder}'' field (if ``\texttt{absorder}'' field has
       a value)
 \item Value of ``\texttt{author}'' (or ``\texttt{editor}'') field. If
       ``\texttt{sortname}'' field is assigned value, it is used instead of
       ``\texttt{author}'' and ``\texttt{editor}''.
...
If ``\texttt{sortname}'' field is assigned value, it is used for sorting instead
of ``\texttt{author}'' or ``\texttt{editor}''.

For example, the following bibliography item is listed after entries with
authors whose family names begin with letter C.
\begin{Frame}
\begin{verbatim}
...
However, ``d'Aspremont'' should usually be listed as author whose name begin
with letter A.  In this case, you had better to add the following
...
Then, this entry is listed as author whose name begin with letter A.
